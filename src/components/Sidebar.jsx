      ...
      <div className="w-fullF bg-yellow-400F ml-0 mt-8 h-80">
        ...
        <div className="mb-8 flex w-full flex-grow flex-wrap">
          {randomTags.map((item, index) => {
            return (
              <div
                key={'shortid_' + index}
                className="box_radius borderF borderF shadowF mr-2 mb-2 cursor-pointer border bg-white px-2 pb-1 pt-0.5 text-sm font-semibold"
              >
                {item}
              </div>
            )
          ...
          {shortBlogList2.map((item, index) => {
            return (
              <Link href={'/post'} passHref key={'shortid_' + index}>
                <div className="card_container relative mb-6 flex w-full flex-grow cursor-pointer select-none flex-row overflow-hidden">
        ...
        <div className="mt-8 w-full">
          ...
          {shortBlogList.map((item, index) => {
            return (
              <Link href={'/post'} passHref key={'shortid_' + index}>
                <div className="card_container relative mb-6 w-full flex-grow cursor-pointer select-none overflow-hidden">
                  <div className="box_radius relative top-0 left-0 mr-3 h-40 w-full overflow-hidden">
                    <img
                  ...
                  <div className="px-2F relative w-full flex-grow pt-2">
                    {/* <div className=" text_accent_colorF text-xs font-semibold text-gray-500">
                      Community
                    </div> */}
                    <div className="blog_title_medium font_styleF bold_text_color card_title mt-1 text-sm font-semibold leading-snug">
                      {item.title}
                    </div>
                  </div>
                </div>
            ...
            return (
              <Link href={'/post'} passHref key={'shortid_' + index}>
                <div className="card_container bg-whiteF relative mb-6 flex w-full flex-grow cursor-pointer select-none flex-row overflow-hidden pb-1">
                  <div className="box_radius w-fullF relative top-0 left-0 mr-3 h-20 w-20 flex-shrink-0 overflow-hidden">
                    <img
                      className="border-br h-fullF image_visibility absolute left-0 h-20 w-20 object-cover"
                      src={item.image}
                      alt={item.title}
